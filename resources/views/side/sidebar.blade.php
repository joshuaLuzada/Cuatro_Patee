

<!-- drawer init and show -->
<div class="text-center" id="drawer">
    <svg width="32px" height="32px" viewBox="0 0 32 32" xmlns="http://www.w3.org/2000/svg" fill="none"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path stroke="#535358" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8h15M5 16h22M5 24h22M5 11l3-3-3-3"></path> </g></svg>
</div>

<!-- drawer component -->
<div id="drawer-navigation" class="fixed top-0 left-0 z-40 h-screen p-4 overflow-y-auto transition-transform -translate-x-full  w-64 bg-[#4c7b8f]" tabindex="-1" aria-labelledby="drawer-navigation-label">
    <h5 id="drawer-navigation-label" class="text-base font-semibold text-white uppercase ">Menu</h5>
    <button type="button" data-drawer-hide="drawer-navigation" aria-controls="drawer-navigation" class="text-white bg-transparent  rounded-lg text-sm w-8 h-8 absolute top-2.5 end-2.5 inline-flex items-center justify-center hover:bg-white hover:text-[#4c7b8f]" >
      <svg class="w-3 h-3" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 14 14">
         <path stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m1 1 6 6m0 0 6 6M7 7l6-6M7 7l-6 6"/>
      </svg>
      <span class="sr-only">Close menu</span>
   </button>
   <div>
   <br>
   <hr>
   <br>
   <nav >
        <a href="{{route("home")}}" class="flex items-center px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                 <svg class="h-5 w-5 mr-3" viewBox="0 -0.5 25 25" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M9.918 10.0005H7.082C6.66587 9.99708 6.26541 10.1591 5.96873 10.4509C5.67204 10.7427 5.50343 11.1404 5.5 11.5565V17.4455C5.5077 18.3117 6.21584 19.0078 7.082 19.0005H9.918C10.3341 19.004 10.7346 18.842 11.0313 18.5502C11.328 18.2584 11.4966 17.8607 11.5 17.4445V11.5565C11.4966 11.1404 11.328 10.7427 11.0313 10.4509C10.7346 10.1591 10.3341 9.99708 9.918 10.0005Z" stroke="#2c3747" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M9.918 4.0006H7.082C6.23326 3.97706 5.52559 4.64492 5.5 5.4936V6.5076C5.52559 7.35629 6.23326 8.02415 7.082 8.0006H9.918C10.7667 8.02415 11.4744 7.35629 11.5 6.5076V5.4936C11.4744 4.64492 10.7667 3.97706 9.918 4.0006Z" stroke="#2c3747" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M15.082 13.0007H17.917C18.3333 13.0044 18.734 12.8425 19.0309 12.5507C19.3278 12.2588 19.4966 11.861 19.5 11.4447V5.55666C19.4966 5.14054 19.328 4.74282 19.0313 4.45101C18.7346 4.1592 18.3341 3.9972 17.918 4.00066H15.082C14.6659 3.9972 14.2654 4.1592 13.9687 4.45101C13.672 4.74282 13.5034 5.14054 13.5 5.55666V11.4447C13.5034 11.8608 13.672 12.2585 13.9687 12.5503C14.2654 12.8421 14.6659 13.0041 15.082 13.0007Z" stroke="#2c3747" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path fill-rule="evenodd" clip-rule="evenodd" d="M15.082 19.0006H17.917C18.7661 19.0247 19.4744 18.3567 19.5 17.5076V16.4936C19.4744 15.6449 18.7667 14.9771 17.918 15.0006H15.082C14.2333 14.9771 13.5256 15.6449 13.5 16.4936V17.5066C13.525 18.3557 14.2329 19.0241 15.082 19.0006Z" stroke="#2c3747" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                  
                <span class="">Dashboard</span>
            </a>
            <a href="{{route("sales")}}" class="flex items-center px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                <svg fill="#2c3747"   class="h-5 w-5 mr-3" viewBox="0 0 1024 1024" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"><path d="M136.948 908.811c5.657 0 10.24-4.583 10.24-10.24V610.755c0-5.657-4.583-10.24-10.24-10.24h-81.92a10.238 10.238 0 00-10.24 10.24v287.816c0 5.657 4.583 10.24 10.24 10.24h81.92zm0 40.96h-81.92c-28.278 0-51.2-22.922-51.2-51.2V610.755c0-28.278 22.922-51.2 51.2-51.2h81.92c28.278 0 51.2 22.922 51.2 51.2v287.816c0 28.278-22.922 51.2-51.2 51.2zm278.414-40.96c5.657 0 10.24-4.583 10.24-10.24V551.322c0-5.657-4.583-10.24-10.24-10.24h-81.92a10.238 10.238 0 00-10.24 10.24v347.249c0 5.657 4.583 10.24 10.24 10.24h81.92zm0 40.96h-81.92c-28.278 0-51.2-22.922-51.2-51.2V551.322c0-28.278 22.922-51.2 51.2-51.2h81.92c28.278 0 51.2 22.922 51.2 51.2v347.249c0 28.278-22.922 51.2-51.2 51.2zm278.414-40.342c5.657 0 10.24-4.583 10.24-10.24V492.497c0-5.651-4.588-10.24-10.24-10.24h-81.92c-5.652 0-10.24 4.589-10.24 10.24v406.692c0 5.657 4.583 10.24 10.24 10.24h81.92zm0 40.96h-81.92c-28.278 0-51.2-22.922-51.2-51.2V492.497c0-28.271 22.924-51.2 51.2-51.2h81.92c28.276 0 51.2 22.929 51.2 51.2v406.692c0 28.278-22.922 51.2-51.2 51.2zm278.414-40.958c5.657 0 10.24-4.583 10.24-10.24V441.299c0-5.657-4.583-10.24-10.24-10.24h-81.92a10.238 10.238 0 00-10.24 10.24v457.892c0 5.657 4.583 10.24 10.24 10.24h81.92zm0 40.96h-81.92c-28.278 0-51.2-22.922-51.2-51.2V441.299c0-28.278 22.922-51.2 51.2-51.2h81.92c28.278 0 51.2 22.922 51.2 51.2v457.892c0 28.278-22.922 51.2-51.2 51.2zm-6.205-841.902C677.379 271.088 355.268 367.011 19.245 387.336c-11.29.683-19.889 10.389-19.206 21.679s10.389 19.889 21.679 19.206c342.256-20.702 670.39-118.419 964.372-284.046 9.854-5.552 13.342-18.041 7.79-27.896s-18.041-13.342-27.896-7.79z"></path><path d="M901.21 112.64l102.39.154c11.311.017 20.494-9.138 20.511-20.449s-9.138-20.494-20.449-20.511l-102.39-.154c-11.311-.017-20.494 9.138-20.511 20.449s9.138 20.494 20.449 20.511z"></path><path d="M983.151 92.251l-.307 101.827c-.034 11.311 9.107 20.508 20.418 20.542s20.508-9.107 20.542-20.418l.307-101.827c.034-11.311-9.107-20.508-20.418-20.542s-20.508 9.107-20.542 20.418z"></path></g></svg>
                   
                <span class="">Sales</span>
            </a>
            
            <a  href="{{route("products.index")}}" class="flex items-center  px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                <svg class="h-5 w-5 mr-3" viewBox="0 0 48 48" fill="none" xmlns="http://www.w3.org/2000/svg" stroke="#4c7b8f">
                    <path d="M48 0H0V48H48V0Z" fill="white" fill-opacity="0.01"></path>
                    <path d="M44 14L24 4L4 14V34L24 44L44 34V14Z" stroke="#2c3747" stroke-width="4" stroke-linejoin="round"></path>
                    <path d="M4 14L24 24" stroke="#4c7b8f" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"></path>
                    <path d="M24 44V24" stroke="#2c3747" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"></path>
                    <path d="M44 14L24 24" stroke="#2c3747" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"></path>
                    <path d="M34 9L14 19" stroke="#2c3747" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg>

                <span>Products</span>
               
                 
            </a>
           
        
           
           

            <a href="{{route("inventory")}}" class="flex items-center px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
               <svg class="h-5 w-5 mr-2" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="#2c3747"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <defs> <path d="M19.6554561,20.9177505 L20.1324211,21.5082919 C19.7289347,21.8246698 19.2287757,22 18.7024516,22 C18.5957479,22 18.4898379,21.9928085 18.385261,21.9785596 L18.4901851,21.2309569 C18.5600191,21.240472 18.6308721,21.245283 18.7024516,21.245283 C19.0539479,21.245283 19.3863488,21.1287603 19.6554561,20.9177505 Z M13.2918622,17.3791385 C14.5579751,17.3791385 15.5843624,18.3904426 15.5843624,19.6379494 C15.5843624,20.8854563 14.5579751,21.8967604 13.2918622,21.8967604 C12.0257494,21.8967604 10.9993621,20.8854563 10.9993621,19.6379494 C10.9993621,18.3904426 12.0257494,17.3791385 13.2918622,17.3791385 Z M17.2015818,20.040198 C17.2869873,20.451794 17.5449795,20.8093683 17.9081786,21.0268796 L17.510337,21.6718118 C16.9661345,21.3459018 16.579504,20.8900362 16.4511322,20.1913721 L17.2015818,20.040198 Z M10.3971131,2 C11.5991439,2 12.616247,2.76528302 13.0045954,3.82210243 L13.0045954,3.82210243 L15.9449479,3.82210243 C16.9620509,3.82210243 17.7942261,4.64204852 17.7942261,5.64420485 L17.7942261,5.64420485 L17.7942261,11.1105121 L15.9449479,11.1105121 L15.9449479,5.64420485 L14.0956696,5.64420485 L14.0956696,8.37735849 L6.69855654,8.37735849 L6.69855654,5.64420485 L4.84927827,5.64420485 L4.84927827,19.309973 L9.47247394,19.309973 L9.47247394,21.1320755 L4.84927827,21.1320755 C3.83217522,21.1320755 3,20.3121294 3,19.309973 L3,19.309973 L3,5.64420485 C3,4.64204852 3.83217522,3.82210243 4.84927827,3.82210243 L4.84927827,3.82210243 L7.78963072,3.82210243 C8.17797915,2.76528302 9.1950822,2 10.3971131,2 Z M20.8598019,18.9546151 C20.9472125,19.1888929 20.9948768,19.436931 20.9999984,19.6977675 C21.0004225,20.0933426 20.9155279,20.4469131 20.7476245,20.7690374 L20.0662579,20.4242448 C20.1778966,20.2900648 20.2343216,19.9750652 20.2340973,19.7055583 C20.2307241,19.5358936 20.1990081,19.370848 20.1408704,19.2150275 L20.8598019,18.9546151 Z M17.5210599,17.793504 L17.9153266,18.440564 C17.550937,18.6561162 17.2909665,19.0123847 17.203282,19.4234195 L16.4536885,19.2681768 C16.5854849,18.6503604 16.9750506,18.1164922 17.5210599,17.793504 Z M18.7024516,17.4716981 C19.350097,17.4716981 19.9554053,17.7375693 20.3872523,18.1961485 L19.8257551,18.7094874 C19.5373774,18.4032585 19.1347584,18.2264151 18.7024516,18.2264151 C18.6390357,18.2269825 18.5619322,18.2313948 18.4998951,18.2394494 L18.3998271,17.4912006 C18.4927197,17.4791399 18.6078739,17.4725444 18.7024516,17.4716981 Z M18.5597349,12.1886792 C19.8258477,12.1886792 20.852235,13.1999834 20.852235,14.4474902 C20.852235,15.6949971 19.8258477,16.7063012 18.5597349,16.7063012 C17.293622,16.7063012 16.2672347,15.6949971 16.2672347,14.4474902 C16.2672347,13.1999834 17.293622,12.1886792 18.5597349,12.1886792 Z M13.2918622,12.1886792 C14.5579751,12.1886792 15.5843624,13.1999834 15.5843624,14.4474902 C15.5843624,15.6949971 14.5579751,16.7063012 13.2918622,16.7063012 C12.0257494,16.7063012 10.9993621,15.6949971 10.9993621,14.4474902 C10.9993621,13.1999834 12.0257494,12.1886792 13.2918622,12.1886792 Z M10.3971131,3.82210243 C9.88856155,3.82210243 9.47247394,4.23207547 9.47247394,4.73315364 C9.47247394,5.23423181 9.88856155,5.64420485 10.3971131,5.64420485 C10.9056646,5.64420485 11.3217522,5.23423181 11.3217522,4.73315364 C11.3217522,4.23207547 10.9056646,3.82210243 10.3971131,3.82210243 Z" id="path-1"> </path> </defs> <g stroke="none" stroke-width="1" fill="none" fill-rule="evenodd"> <mask id="mask-2" fill="white"> <use xlink:href="#path-1"> </use> </mask> <use fill="#2c3747" fill-rule="nonzero" xlink:href="#path-1"> </use> </g> </g></svg>
                    
                <span class="">Inventory</span>
            </a>
            <a href="{{route("orders")}}" class="flex items-center px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                <svg class="h-5 w-5 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fill-rule="evenodd" clip-rule="evenodd" d="M16.5 6H3V17.25H3.375H4.5H4.52658C4.70854 18.5221 5.80257 19.5 7.125 19.5C8.44743 19.5 9.54146 18.5221 9.72342 17.25H15.0266C15.2085 18.5221 16.3026 19.5 17.625 19.5C18.9474 19.5 20.0415 18.5221 20.2234 17.25H21.75V12.4393L18.3107 9H16.5V6ZM16.5 10.5V14.5026C16.841 14.3406 17.2224 14.25 17.625 14.25C18.6721 14.25 19.5761 14.8631 19.9974 15.75H20.25V13.0607L17.6893 10.5H16.5ZM15 15.75V9V7.5H4.5V15.75H4.75261C5.17391 14.8631 6.07785 14.25 7.125 14.25C8.17215 14.25 9.07609 14.8631 9.49739 15.75H15ZM17.625 18C17.0037 18 16.5 17.4963 16.5 16.875C16.5 16.2537 17.0037 15.75 17.625 15.75C18.2463 15.75 18.75 16.2537 18.75 16.875C18.75 17.4963 18.2463 18 17.625 18ZM8.25 16.875C8.25 17.4963 7.74632 18 7.125 18C6.50368 18 6 17.4963 6 16.875C6 16.2537 6.50368 15.75 7.125 15.75C7.74632 15.75 8.25 16.2537 8.25 16.875Z" fill="#2c3747"></path> </g></svg>
                   
                <span class="">Orders</span>
            </a>
            <br>
            <p class="text-white py-4 ">Documentation</p>
            <hr>
            <br>
            <a href="{{route("reports")}}" class="flex items-center px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                 <svg fill="#2c3747" class="h-5 w-5 mr-2" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <path d="M94.729,392.533c3.968,19.448,21.205,34.133,41.805,34.133c22.025,0,39.996-16.845,42.223-38.298l88.849-74.035 c6.263,15.778,21.615,26.999,39.595,26.999c18.918,0,34.807-12.459,40.388-29.534l79.616,43.426 c2.671,20.975,20.437,37.308,42.129,37.308c23.526,0,42.667-19.14,42.667-42.667c0-23.526-19.14-42.667-42.667-42.667 c-18.918,0-34.807,12.459-40.388,29.534l-79.616-43.426C346.658,272.333,328.892,256,307.2,256 c-22.025,0-40.004,16.845-42.223,38.298l-88.849,74.035c-6.263-15.778-21.615-27-39.595-27c-20.599,0-37.837,14.686-41.805,34.133 H34.133v-34.133h8.533c4.719,0,8.533-3.814,8.533-8.533s-3.814-8.533-8.533-8.533h-8.533v-25.6h8.533 c4.719,0,8.533-3.814,8.533-8.533c0-4.719-3.814-8.533-8.533-8.533h-6.69l69.53-900.429c6.554,4.087,14.216,6.562,22.494,6.562 c13.585,0,25.574-6.502,33.391-16.427l45.022,22.511c-0.947,3.524-1.613,7.159-1.613,10.982c0,23.526,19.14,42.667,42.667,42.667 c23.526,0,42.667-19.14,42.667-42.667c0-9.173-2.97-17.621-7.919-24.593l54.417-101.052c6.391,3.84,13.781,6.178,21.769,6.178 c8.405,0,16.205-2.526,22.818-6.741l55.782,71.714c0.188,0.247,0.469,0.333,0.683,0.555c-6.775,7.552-11.017,17.417-11.017,28.339 c0,23.526,19.14,42.667,42.667,42.667c23.526,0,42.667-19.14,42.667-42.667c0-23.526-19.14-42.667-42.667-42.667 c-6.596,0-12.774,1.63-18.355,4.318c-0.196-0.333-0.265-0.708-0.512-1.024L393.6,66.714c4.693-6.861,7.467-15.13,7.467-24.047 C401.067,19.14,381.926,0,358.4,0c-23.526,0-42.667,19.14-42.667,42.667c0,9.173,2.97,17.621,7.919,24.593l-54.417,101.052 c-6.391-3.84-13.781-6.178-21.769-6.178c-13.585,0-25.574,6.502-33.391,16.427l-45.022-22.511 c0.947-3.524,1.613-7.159,1.613-10.982c0-23.526-19.14-42.667-42.667-42.667c-23.526,0-42.667,19.14-42.667,42.667 c0,9.011,2.842,17.348,7.629,24.243L44.501,239.3c-0.606-0.137-1.186-0.367-1.835-0.367h-8.533v-25.6h8.533 c4.719,0,8.533-3.814,8.533-8.533s-3.814-8.533-8.533-8.533h-8.533v-25.6h8.533c4.719,0,8.533-3.814,8.533-8.533 s-3.814-8.533-8.533-8.533h-8.533V128h8.533c4.719,0,8.533-3.814,8.533-8.533s-3.814-8.533-8.533-8.533h-8.533v-25.6h8.533 c4.719,0,8.533-3.814,8.533-8.533s-3.814-8.533-8.533-8.533h-8.533v-25.6h8.533c4.719,0,8.533-3.814,8.533-8.533 S47.386,25.6,42.667,25.6h-8.533V8.533C34.133,3.814,30.319,0,25.6,0s-8.533,3.814-8.533,8.533V25.6H8.533 C3.814,25.6,0,29.414,0,34.133s3.814,8.533,8.533,8.533h8.533v25.6H8.533C3.814,68.267,0,72.081,0,76.8s3.814,8.533,8.533,8.533 h8.533v25.6H8.533c-4.719,0-8.533,3.814-8.533,8.533S3.814,128,8.533,128h8.533v25.6H8.533C3.814,153.6,0,157.414,0,162.133 s3.814,8.533,8.533,8.533h8.533v25.6H8.533C3.814,196.267,0,200.081,0,204.8s3.814,8.533,8.533,8.533h8.533v25.6H8.533 c-4.719,0-8.533,3.814-8.533,8.533S3.814,256,8.533,256h8.533v25.6H8.533C3.814,281.6,0,285.414,0,290.133 c0,4.719,3.814,8.533,8.533,8.533h8.533v25.6H8.533C3.814,324.267,0,328.081,0,332.8s3.814,8.533,8.533,8.533h8.533v25.6H8.533 c-4.719,0-8.533,3.814-8.533,8.533S3.814,384,8.533,384h8.533v25.6H8.533C3.814,409.6,0,413.414,0,418.133 s3.814,8.533,8.533,8.533h8.533v25.6H8.533C3.814,452.267,0,456.081,0,460.8c0,4.719,3.814,8.533,8.533,8.533h8.533V486.4 c0,4.719,3.814,8.533,8.533,8.533h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533s8.533-3.814,8.533-8.533v-8.533h25.6v8.533 c0,4.719,3.814,8.533,8.533,8.533s8.533-3.814,8.533-8.533v-8.533h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533 s8.533-3.814,8.533-8.533v-8.533h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533s8.533-3.814,8.533-8.533v-8.533h25.6v8.533 c0,4.719,3.814,8.533,8.533,8.533s8.533-3.814,8.533-8.533v-8.533h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533 s8.533-3.814,8.533-8.533v-8.533h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533s8.533-3.814,8.533-8.533v-8.533h25.6v8.533 c0,4.719,3.814,8.533,8.533,8.533s8.533-3.814,8.533-8.533v-8.533h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533 c4.719,0,8.533-3.814,8.533-8.533v-8.533h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533c4.719,0,8.533-3.814,8.533-8.533v-8.533 h25.6v8.533c0,4.719,3.814,8.533,8.533,8.533s8.533-3.814,8.533-8.533v-8.533h8.533c4.719,0,8.533-3.814,8.533-8.533 s-3.814-8.533-8.533-8.533h-8.533v-8.533c0-4.719-3.814-8.533-8.533-8.533s-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533 c0-4.719-3.814-8.533-8.533-8.533c-4.719,0-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533c0-4.719-3.814-8.533-8.533-8.533 c-4.719,0-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533c0-4.719-3.814-8.533-8.533-8.533s-8.533,3.814-8.533,8.533v8.533h-25.6 v-8.533c0-4.719-3.814-8.533-8.533-8.533s-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533c0-4.719-3.814-8.533-8.533-8.533 s-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533c0-4.719-3.814-8.533-8.533-8.533s-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533 c0-4.719-3.814-8.533-8.533-8.533s-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533c0-4.719-3.814-8.533-8.533-8.533 s-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533c0-4.719-3.814-8.533-8.533-8.533s-8.533,3.814-8.533,8.533v8.533h-25.6v-8.533 c0-4.719-3.814-8.533-8.533-8.533s-8.533,3.814-8.533,8.533v8.533H34.133v-85.333H94.729z"></path> </g> </g> </g></svg>
                    
                <span class="">Reports</span>
            </a>

            <a href="{{route("receipt")}}" class="flex items-center px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                <svg  class="h-5 w-5 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M17 13.0001H21V19.0001C21 20.1047 20.1046 21.0001 19 21.0001M17 13.0001V19.0001C17 20.1047 17.8954 21.0001 19 21.0001M17 13.0001V5.75719C17 4.8518 17 4.3991 16.8098 4.13658C16.6439 3.90758 16.3888 3.75953 16.1076 3.72909C15.7853 3.6942 15.3923 3.9188 14.6062 4.368L14.2938 4.54649C14.0045 4.71183 13.8598 4.7945 13.7062 4.82687C13.5702 4.85551 13.4298 4.85551 13.2938 4.82687C13.1402 4.7945 12.9955 4.71183 12.7062 4.54649L10.7938 3.45372C10.5045 3.28838 10.3598 3.20571 10.2062 3.17334C10.0702 3.14469 9.92978 3.14469 9.79383 3.17334C9.64019 3.20571 9.49552 3.28838 9.20618 3.45372L7.29382 4.54649C7.00448 4.71183 6.85981 4.7945 6.70617 4.82687C6.57022 4.85551 6.42978 4.85551 6.29383 4.82687C6.14019 4.7945 5.99552 4.71183 5.70618 4.54649L5.39382 4.368C4.60772 3.9188 4.21467 3.6942 3.89237 3.72909C3.61123 3.75953 3.35611 3.90758 3.1902 4.13658C3 4.3991 3 4.8518 3 5.75719V16.2001C3 17.8803 3 18.7203 3.32698 19.3621C3.6146 19.9266 4.07354 20.3855 4.63803 20.6731C5.27976 21.0001 6.11984 21.0001 7.8 21.0001H19M7 13.0001H9M7 9.0001H13M7 17.0001H9M13 17.0001H13.01M13 13.0001H13.01" stroke="#2c3747" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                <span class="">Receipts</span>
            </a>
            <br>
            <p class="text-white py-4 ">Access</p>
            <hr>
            <br>
            @if(session('username') === 'admin123')
            <a href="{{ route('accounts.index') }}" class="flex items-center px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                <svg fill="#2c3747" class="h-5 w-5 mr-2" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" viewBox="0 0 512 512" xml:space="preserve"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <g> <g> <path d="M352.062,314.075c-19.834-20.912-43.665-36.124-68.765-44.408c25.008-22.555,40.754-55.191,40.754-91.439 c0-67.899-55.24-123.139-123.139-123.139S77.772,110.329,77.772,178.228c0,36.248,15.746,68.884,40.754,91.439 c-25.101,8.285-48.932,23.498-68.766,44.409C17.672,347.906,0,391.728,0,437.468v19.443h401.823v-19.443 C401.823,391.728,384.15,347.906,352.062,314.075z M116.658,178.228c0-46.457,37.796-84.253,84.253-84.253 c46.457,0,84.253,37.796,84.253,84.253s-37.796,84.253-84.253,84.253C154.454,262.481,116.658,224.685,116.658,178.228z M40.256,418.025c9.65-67.94,68.591-116.658,121.769-116.658h77.772c53.178,0,112.119,48.718,121.769,116.658H40.256z"></path> </g> </g> <g> <g> <polygon points="453.671,223.595 453.671,165.266 414.785,165.266 414.785,223.595 356.456,223.595 356.456,262.481 414.785,262.481 414.785,320.81 453.671,320.81 453.671,262.481 512,262.481 512,223.595 "></polygon> </g> </g> </g></svg>
                    
                <span class="">Manage Accounts</span>
            </a>
            @endif
            <div class="justify-end ">
                <a href="/" class="flex items-center  px-4 py-2 text-white hover:bg-white hover:text-[#4c7b8f] rounded">
                <svg class="h-5 w-5 mr-2" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" stroke-width="0"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M12 12H19M19 12L16 15M19 12L16 9" stroke="#2c3747" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> <path d="M19 6V5C19 3.89543 18.1046 3 17 3H7C5.89543 3 5 3.89543 5 5V19C5 20.1046 5.89543 21 7 21H17C18.1046 21 19 20.1046 19 19V18" stroke="#2c3747" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path> </g></svg>
                        
                <span class="">Logout</span>
            </a>
            </div>
            
            
            
        </nav>
    </div>
    

    
</div>

<script>
   ;
        document.addEventListener("DOMContentLoaded", function () {
        const drawer = document.getElementById("drawer-navigation");
        const drawerToggle = document.getElementById("drawer");
        const drawerClose = document.querySelector("[data-drawer-hide]");

        // Function to toggle the drawer
        function toggleDrawer() {
            drawer.classList.toggle("-translate-x-full");
        }

        // Open drawer when clicking the toggle button
        drawerToggle.addEventListener("click", function (event) {
            event.stopPropagation(); // Prevent click from propagating to the document
            toggleDrawer();
        });

        // Close drawer when clicking the close button
        drawerClose.addEventListener("click", toggleDrawer);

        // Close drawer when clicking outside of it
        document.addEventListener("click", function (event) {
            if (!drawer.contains(event.target) && !drawerToggle.contains(event.target)) {
                if (!drawer.classList.contains("-translate-x-full")) {
                    toggleDrawer();
                }
            }
        });
    });
</script>
